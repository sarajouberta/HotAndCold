...
void Space::update() {
	for (auto const& actor : dynamicActors) {
		...
		//if (actor->vy > 20) {
			//actor->vy = 20;
		//} //Retiro apliación de gravedad y límite vertical

        // Aun no se han detectado choques
        actor->collisionDown = false;
        /*outLeft/right = true xq la comprobación si fuera de un únnico bloque
          no puede determinar que el actor tiene un "pie en el aire": ese bloque sabe que NO es pisado, pero no sabe si en otro sí pisa.
          un único bloque NUNCA puede poner estas var a true
          lo que sí puede es ponerlos a false: si tiene un pie encima, NO tiene pie en el aire*/
        actor->outLeft = true; 
        actor->outRight = true;
        ...
        for (auto const& staticAct : staticActors) {
            ...
            if ((downDynamic + dynamicAct->vy) >= topStatic
                ...
                if (possibleMovement >= topStatic - downDynamic) {
                    ...
                    //comprobar si medio cuerpo está o no pisando bloque:
                    //(usadas x enemigo para controlar caída)
